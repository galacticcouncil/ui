 ...
 * @returns {HTMLElement}
 */
export function createStyleInShadowElement(cssText: CSSResult, shadowRoot: ShadowRoot): HTMLStyleElement {
  const style = document.createElement('style');
  style.textContent = cssText.cssText;
 ...
 * @param value - optional attribute value to add to tag
 */
export function createAdaptedStyle(cssText: CSSResult, name: string, value: string): void {
...
export function hasAdaptedStyle(name: string, value: string): boolean {
  return !!document.adoptedStyleSheets.find(
    (sh) => (sh as Record<string, any>)._uigcStyleId === getStyleId(name, value)
  );
}
